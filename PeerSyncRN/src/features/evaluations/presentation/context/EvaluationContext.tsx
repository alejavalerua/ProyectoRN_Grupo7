...
interface EvaluationContextProps {
  ...
  saveActivity: (categoryId: string, data: { name: string; description?: string; startDate: Date; endDate: Date; isVisible: boolean }) => Promise<boolean>;
...
export const EvaluationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [activeActivitiesCountByCategory, setActiveActivitiesCountByCategory] = useState<Record<string, number>>({});
  ...
  const getActivityPriority = useCallback((activity: Activity) => {
    const now = new Date();
    if (now > activity.endDate) return 2; // Vencida
    if (now < activity.startDate) return 1; // Pendiente
    return 0; // Activa
  }, []);
  ...
  const getActivityUIData = useCallback((activity: Activity) => {
    ...
    let label = isExpired ? 'Vencida' : isPending ? 'Próximamente' : 'Pendiente';
    let detail = isExpired ? `• Cierre: ${dayStr} ${monthStr} ${timeStr}` : 
                 isPending ? `• Abre: ${dayStr} ${monthStr} ${timeStr}` : 
                 `• Cierre: ${dayStr} ${monthStr} ${timeStr}`;

    return { month: monthStr, day: dayStr, statusLabel: label, statusDetail: detail, isExpired, isPending, isActive };
  }, []);
  ...
  const getTeacherActivityUIData = useCallback((activity: Activity) => {
    ...
    let statusTag = isExpired ? "Finalizada" : isPending ? "Programada" : "En curso";
    let statusDetail = !activity.visibility ? "• Oculta (Borrador)" : `• Cierra ${timeStr}`;
  ...
  const saveActivity = async (categoryId: string, data: { name: string; description?: string; startDate: Date; endDate: Date; isVisible: boolean }) => {
    try {
      setIsSaving(true);
      await repository.createActivity(categoryId, data.name, data.description || '', data.startDate, data.endDate, data.isVisible);
      return true;
    } catch (e: any) {
  ...
  const loadActivities = async (categoryId: string) => {
    ...
    try {
      ...
      setActivities(res);
    } catch (e) { console.log(e); } finally { setIsLoadingActivities(false); }
  };
  ...
  const loadTeacherActivities = async (categoryId: string) => {
    ...
    } catch (e) { console.log(e); } finally { setIsLoadingTeacherActivities(false); }
  };
  ...
  const loadActiveActivitiesCount = async (categoryId: string) => {
    try {
      ...
      setActiveActivitiesCountByCategory(prev => ({ ...prev, [categoryId]: activeCount }));
    } catch (e) { }
  };
  ...
  const loadHomeActivitiesPreview = async (categoryIds: string[]) => {
    ...
    try {
      ...
      all.sort((a, b) => {
        ...
        if (!aExp && !bExp) return Math.abs(a.endDate.getTime() - now.getTime()) - Math.abs(b.endDate.getTime() - now.getTime());
        return b.endDate.getTime() - a.endDate.getTime();
      });

      setHomeActivities(all.slice(0, 3));
    } catch (e) { console.log(e); } finally { setIsLoadingHomeActivities(false); }
  };

  return (
    <EvaluationContext.Provider value={{ activities, teacherActivities, homeActivities, activeActivitiesCountByCategory, isLoadingActivities, isLoadingTeacherActivities, isLoadingHomeActivities, isSaving, loadActivities, loadTeacherActivities, loadActiveActivitiesCount, loadHomeActivitiesPreview, saveActivity, sortedActivities, getActivityUIData, getTeacherActivityUIData, getActiveActivitySubtitle }}>
      {children}
